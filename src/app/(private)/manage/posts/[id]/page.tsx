...
type Params = {
	params: Promise<{id: string}> // 記事のIDを取得
}
...
export default async function ShowPage({params}: Params) {
	...
	const { id } = await params // 記事のIDを取得
			...
			<Card className="max-w-3xl mx-auto pt-0 pb-6">
				{post.topImage && (
					<div className="relative w-full h-64 lg:h-96">
						<Image src={post.topImage} alt={post.title} fill sizes="100vw" className="rounded-t-md object-cover" priority/>
					</div>
				)}
				<CardHeader>
